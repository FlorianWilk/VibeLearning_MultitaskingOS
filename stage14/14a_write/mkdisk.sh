...
nused = 2 + len(files)
# imap/zmap: die ersten nused Bits gesetzt (ueber mehrere Bytes verteilt, da
# nused>8 moeglich), restliche Bytes pauschal als belegt markiert.
...
for off in (2*BLOCK, 3*BLOCK):
    ...
    for i in range(full + (1 if rem else 0), BLOCK):
        img[off+i] = 0xFF
INODES = 4 * BLOCK
