...
disk_irq_handler:
    pusha
    mov dx, ATA_STATUS
    in al, dx                      ; Status lesen quittiert den IRQ
    cmp byte [disk_op], 0          ; Write-Complete? dann keine Daten lesen
    jne .done
    mov edi, disk_kbuf
    mov dx, ATA_DATA
    mov ecx, 256
...
; kwrite (eax=sektor, esi=src) -> 512 Byte aus [esi] schreiben (ATA 0x30)
kwrite:
    push ebx
    push ecx
    push edx
    ...
    mov dx, ATA_STATUS
.wd:
    in al, dx                      ; auf DRQ (Bit 0x08) warten
    test al, 0x08
    jz .wd
    mov dx, ATA_DATA
    mov ecx, 256
    cld
    rep outsw                      ; 256 Words aus [esi] in den Datenport
    mov eax, WAIT_DISK
    call block                     ; bis Write-Complete-IRQ uns weckt
    pop edx
    pop ecx
    pop ebx
    ret
...
; ============================================================================
; Allokation (14b): erstes freies Bit in imap/zmap finden, setzen, zurueck.
; Konvention: bit0=Sentinel, bit1=root; imap bit k=Inode k; zmap bit k=Zone
; FIRST_DATA+k-1. imap @Sektor 4, zmap @Sektor 6 (je 1. Sektor genutzt).
; ============================================================================
; alloc_inode -> eax = neue Inode-Nr (1..15), 0 wenn voll
alloc_inode:
    push ebx
    push ecx
    push edx
    push esi
    mov eax, 4                      ; imap-Sektor
    call kread
    mov esi, 1                      ; ab bit 1 (bit0 = Sentinel)
.s:
    cmp esi, 16                     ; nur 1. Inode-Sektor (Inode 1..15)
    jae .full
    mov ebx, esi
    shr ebx, 3                      ; Byte-Index
    mov ecx, esi
    and ecx, 7                      ; Bit-Position 0..7
    mov dl, 1
    shl dl, cl                      ; dl = Maske
    mov al, [disk_kbuf + ebx]
    test al, dl
    jz .found
    inc esi
    jmp .s
.found:
    or al, dl                       ; Bit belegen
    mov [disk_kbuf + ebx], al
    mov [alloc_tmp], esi
    mov eax, 4
    mov esi, disk_kbuf
    call kwrite                     ; imap zurueckschreiben
    mov eax, [alloc_tmp]
    jmp .ret
.full:
    xor eax, eax
...
; alloc_zone -> eax = neue Zone-Nr (absoluter Block), 0 wenn voll
alloc_zone:
    push ebx
    push ecx
    push edx
    push esi
    mov eax, 6                      ; zmap-Sektor
    call kread
    mov esi, 1
.s:
    cmp esi, 127                    ; Zone < N_ZONES (128)
    jae .full
    mov ebx, esi
    shr ebx, 3
    mov ecx, esi
...
.found:
    ...
    mov eax, 6
    mov esi, disk_kbuf
    call kwrite                     ; zmap zurueckschreiben
    mov eax, 2                      ; Superblock fuer FIRST_DATA
    call kread
    movzx eax, word [disk_kbuf + 8] ; s_firstdatazone
    add eax, [alloc_tmp]
    dec eax                         ; Zone = FIRST_DATA + k - 1
    jmp .ret
.full:
    xor eax, eax
.ret:
...
do_readfile:
    ...
    call copy_name
    pop edi
    call load_program                   ; laedt fname_buf nach edi, setzt saved_size
    test eax, eax
    jz .nf
    mov eax, [saved_size]
    mov [esp + 28], eax
...
; Overwrite-Modell: schreibt in die existierenden Zonen der Datei (max 7).
do_writefile:
    mov [io_dst], ecx
...
.wdone:
    ...
    mov dword [esp + 28], 0
    jmp syscall_dispatch.ret
; ---- Datei existiert nicht -> neu anlegen (14b), dann gemeinsamer Schreibpfad
.nf:
    call alloc_inode
    test eax, eax
    jz .err
    ...
    test eax, eax
    jz .err
    mov [saved_zones], eax              ; Zone[0] = neue Zone, Rest 0
    xor ebx, ebx
    mov [saved_zones + 4], ebx
    mov [saved_zones + 8], ebx
    mov [saved_zones + 12], ebx
    mov [saved_zones + 16], ebx
    mov [saved_zones + 20], ebx
    mov [saved_zones + 24], ebx
    ; --- Inode N anlegen (Mode + i_zone[0]); i_size setzt der Schreibpfad ---
    mov eax, [inode_table_sector]
    call kread
    mov eax, [found_inode]
    dec eax
    ...
    xor eax, eax
    cld
    rep stosd                           ; Inode (32B) nullen
    pop edi
    mov word [edi + 0], 0x81ED          ; i_mode = regular (0o100755)
    mov eax, [saved_zones]
    mov [edi + 14], ax                  ; i_zone[0]
    mov eax, [inode_table_sector]
    mov esi, disk_kbuf
    call kwrite
    ; --- Directory-Eintrag in root anhaengen ---
    mov eax, [inode_table_sector]
    call kread
    movzx eax, word [disk_kbuf + 14]    ; root i_zone[0]
    shl eax, 1
    mov [root_dir_sector], eax
    call kread                          ; Root-Dir laden
    mov esi, disk_kbuf
    xor ecx, ecx
.df:
    cmp ecx, 32
    jae .err                            ; Directory voll
    movzx eax, word [esi]
    test eax, eax
    jz .dfound
    add esi, 16
...
.dfound:
    mov eax, [found_inode]
    mov [esi], ax                       ; dirent.inode
    lea edi, [esi + 2]                  ; dirent.name
    mov esi, fname_buf
    mov ecx, 14
    cld
    rep movsb
    mov eax, [root_dir_sector]
    mov esi, disk_kbuf
    call kwrite
    ; --- in den gemeinsamen Schreibpfad (schreibt Daten + i_size) ---
    mov eax, [io_param_size]
    mov [io_rem], eax
    mov dword [io_idx], 0
    jmp .wl
...
; ---- listdir(ebx=buf) -> eax=len ; Dateinamen newline-getrennt ------------
do_listdir:
    mov [io_dst], ebx
    mov eax, 2
    call kread
    ...
    add eax, ebx
    shl eax, 1
    call kread                          ; Inode-Tabelle
    movzx eax, word [disk_kbuf + 14]    ; Root i_zone[0]
    shl eax, 1
    call kread                          ; Root-Dir
    mov esi, disk_kbuf
    mov edi, [io_dst]
    xor edx, edx                        ; Gesamtlaenge
    mov dword [ld_i], 32
.l:
    movzx eax, word [esi]
    test eax, eax
...
.cn:
    ...
    jnz .cn
.cndone:
    mov byte [edi], 0x0A               ; Newline nach jedem Namen
    inc edi
    inc edx
.skip:
    add esi, 16
...
.slot:
    ...
    or  eax, 0x007
    mov [ebx + 12], eax
    ; UPT[4] = DATA[slot] | 7  (virt 0x804000 -> Daten-/Puffer-Page)
    mov eax, esi
    imul eax, eax, 0x10000
    add eax, CODE_BASE + 0x4000
    or  eax, 0x007
    mov [ebx + 16], eax
...
io_param_size      dd 0
alloc_tmp          dd 0
root_dir_sector    dd 0
disk_op            db 0
zbuf               times 1024 db 0
exec_slot          dd 0
exec_bg            dd 0
exec_cmdline       dd 0
exec_cmdlen        dd 0
