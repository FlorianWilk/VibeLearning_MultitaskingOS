...
files = [
    ...
    ('cat',   rd(b + '/cat.bin')),
    ('ls',    rd(b + '/ls.bin')),
    ('write', rd(b + '/write.bin')),
    ('gruss.txt', b'Hallo aus gruss.txt!\nMit cat gelesen.\n'),
    ('notiz.txt', b'(leer)\n'),       # beschreibbar (1 Zone) -> write notiz.txt ...
]
...
nused = 2 + len(files)
# imap/zmap: echte Minix-Semantik -- nur die ersten nused Bits belegt (Sentinel
# bit0 + root + Dateien), Rest 0 = frei. So findet die Kernel-Allokation (14b)
# freie Inodes/Zonen. Der Lese-Pfad konsultiert die Bitmap nie.
full, rem = divmod(nused, 8)
for off in (2*BLOCK, 3*BLOCK):
    for k in range(full):
        img[off+k] = 0xFF
    if rem:
        img[off+full] = (1 << rem) - 1
INODES = 4 * BLOCK
